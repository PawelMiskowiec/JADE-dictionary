...
public class ServiceAgent extends Agent {
	protected void setup () {
		...
		addBehaviour(new DevilCyclicBehaviour(this));
		//doDelete();
	}
...
class DevilCyclicBehaviour extends CyclicBehaviour {
	ServiceAgent agent;

	public DevilCyclicBehaviour(ServiceAgent agent)
	{
		this.agent = agent;
	}
	public void action()
	{
		MessageTemplate template = MessageTemplate.MatchOntology("devil");
		ACLMessage message = agent.receive(template);
		if (message == null)
		{
			block();
		...
		else
		{
			...
			try
			{
				response = agent.makeRequest("devil", content);
			}
			catch (NumberFormatException ex)
			{
				response = ex.getMessage();
